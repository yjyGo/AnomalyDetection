...
for i in range(0,maxcolumn_num):
    v = column[i]
    MI.append(mutual_info_score(X_train[v],X_train[v]))
...
for i in range(maxcolumn_num):
    v1 = column[i]
    emd = alpha*emd_mi_matrix[v1][v1]
    js = emd-(1-alpha)*mutual_info_score(X_train[v1],X_train[v1])
    if js >= max_j:
        max_j = js
        max_v1 = v1
selected_feature = [max_v1]
...
# print(alpha)
for n in range(maxcolumn_num-1): #select 3,4,...,maxcolumn features
    max_j = float('-inf')
    ...
    for i in range(maxcolumn_num):
        ...
        # print('emd',com_emd(selected_feature, vi),end=',')
        # print('mi',com_mi(selected_feature, vi))
...
# X_new = X.values
# sss = StratifiedShuffleSplit(n_splits=1, test_size=0.8, random_state=42)
# for i in range(1,maxcolumn_num+1):
